...
class CSVWriter(FileWriter):
    ...
    def _data_mapping(self) -> dict[str, Any]:
        data_mapping = {
            ...
            # Добавленные поля для этажности и планов этажей и ОГРН
            'floors_count': 'Этажность', 'floor_id': 'ID плана этажа', 'floor_plans': 'Планы этажей (names)',
            'address_floor': 'Этаж (из address_comment)', 'ogrn': 'ОГРН/ОГРНИП'
        }
    ...
    def _find_ogrn_in_obj(self, obj: Any) -> str | None:
        """
        Recursively search `obj` (dict/list/str) for substrings that contain OGRN/OGRNIP or
        sequences of 13/15 digits and return the first matched numeric string (digits only).
        """
        ogrn_pattern = re.compile(r'\b(ОГРН|ОГРНИП)\b\D*([0-9]{13}|[0-9]{15})', flags=re.IGNORECASE)
        digits_pattern = re.compile(r'([0-9]{13}|[0-9]{15})')

        def search(x: Any) -> str | None:
            if x is None:
                return None
            if isinstance(x, str):
                m = ogrn_pattern.search(x)
                if m:
                    return m.group(2)
                m2 = digits_pattern.search(x)
                if m2:
                    return m2.group(1)
                return None
            if isinstance(x, dict):
                # check likely fields first for speed
                for k in ('text_warning', 'legal_name', 'text', 'ads', 'name', 'description'):
    ...
    def set_building_info(self, **kwargs) -> None:
        """Set building-level information that should be applied to all items from this building."""
        # Store building floors if provided
        if 'floors' in kwargs:
            self._building_floors = kwargs['floors']
        if 'building_floors' in kwargs:
            self._building_floors = kwargs['building_floors']
    ...
    def _extract_raw(self, catalog_doc: Any) -> dict[str, Any]:
        ...
        # -----------------------------
        # Floors count & floor plans extraction
        # -----------------------------
        # Priority:
        # 1) self._building_floors (from the building page itself)
        # 2) item['floors']['ground_count']
        # 3) catalog_item.floors (various representations)
        # 4) parse digits from address_comment (e.g., "1 этаж")
        
        # First priority: use building floors if provided (from inside/... page)
        if hasattr(self, '_building_floors') and self._building_floors is not None:
            data['floors_count'] = self._building_floors
        elif isinstance(item, dict):
            # raw floors in item
            if 'floors' in item and isinstance(item['floors'], dict) and 'ground_count' in item['floors']:
                data['floors_count'] = item['floors']['ground_count']

            # floor_id
            if 'floor_id' in item:
                data['floor_id'] = item['floor_id']

            # floor_plans: collect names if present (comma-separated)
            if 'floor_plans' in item and isinstance(item['floor_plans'], dict):
                plans = item['floor_plans'].get('plans') or []
                if plans:
                    # use plan names (e.g., "-1","0","1",...)
                    names = [p.get('name') for p in plans if isinstance(p, dict) and 'name' in p]
                    data['floor_plans'] = self._options.csv.join_char.join(names) if names else None

        # try catalog_item.floors alternative representations
        floors_obj = getattr(catalog_item, 'floors', None)
        if floors_obj is not None and not data.get('floors_count'):
            if hasattr(floors_obj, 'ground_count') and getattr(floors_obj, 'ground_count') is not None:
                data['floors_count'] = getattr(floors_obj, 'ground_count')
            elif isinstance(floors_obj, dict) and 'ground_count' in floors_obj:
                data['floors_count'] = floors_obj['ground_count']
            elif hasattr(floors_obj, 'value') and hasattr(floors_obj.value, 'ground_count'):
                data['floors_count'] = getattr(floors_obj.value, 'ground_count')

        # fallback: parse "N этаж" from address_comment
        if not data.get('floors_count') and data.get('address_comment'):
            m = re.search(r'(\d{1,2})\s*(?:этаж|эт\.)', data['address_comment'], flags=re.IGNORECASE)
            if m:
                data['address_floor'] = m.group(1)
                # optionally set floors_count if it's clear this is building floors (careful: often this is point floor)
                # we do not override floors_count here to avoid false positives

        # -----------------------------
        # OGRN / OGRNIP extraction (scan many likely places)
        # -----------------------------
        ogrn_val = None
        # check item-level common fields
        if isinstance(item, dict):
            # direct text_warning at item
            if 'text_warning' in item and item['text_warning']:
                text_warning = item['text_warning']
                # Ищем ОГРН или ОГРНИП в тексте
                ogrn_match = re.search(r'(ОГРН|ОГРНИП)\s+([0-9]+)', text_warning)
                if ogrn_match:
                    ogrn_val = ogrn_match.group(2)
            # ads in item (sometimes)
            if not ogrn_val and 'ads' in item:
                ogrn_val = self._find_ogrn_in_obj(item['ads'])

        # also check catalog_doc.result.ad block (global ads / banners)
        if not ogrn_val:
            result = catalog_doc.get('result') if isinstance(catalog_doc, dict) else None
            if isinstance(result, dict):
                ad_block = result.get('ad') or result.get('ads') or None
                if ad_block:
                    ogrn_val = self._find_ogrn_in_obj(ad_block)

        # as final fallback search whole catalog_doc for any OGRN-like numbers
        if not ogrn_val:
            ogrn_val = self._find_ogrn_in_obj(catalog_doc)

        if ogrn_val:
            # normalize: remove non-digits just in case
            data['ogrn'] = re.sub(r'\D', '', ogrn_val)
